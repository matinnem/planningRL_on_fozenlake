...
#   (1) Policy evaluation  — solve v_π = r_π + γ·P_π·v_π iteratively.
#   (2) Policy improvement — π_{k+1} = argmax_π (r_π + γ·P_π·v_π_k).
pi_s   = {i: 5 for i in state_space}         # initial policy (all "stay")
v_pi_s = {key: 0.0 for key in pi_s}          # value of the current policy
diff1  = 1000
k = 0
gamma = 0.9

print("═" * 60)
print("  Policy Iteration  •  FrozenLake 5×5  (Planning)")
print("═" * 60)
...
while diff1 >= 1e-10:
    # ---- (1) Policy evaluation: iterate Bellman equation to convergence ----
    ...
    while diff2 >= 1e-10:
        for s_first in state_space:
            action = pi_s[s_first]                        # action chosen by π
            num_s_first = state_space[s_first]
            num_s_next, r = transition(num_s_first, action)
            s_next = key_of(num_s_next)
            v_temp = r + gamma * v_pi_js[s_next]          # Bellman expectation
            diff_vector2[s_first] = float(np.abs(v_pi_js[s_first] - v_temp))
    ...
    # ---- (2) Policy improvement: greedy w.r.t. v_pi_s ----------------------
    for s_first in state_space:
        q_values = []
