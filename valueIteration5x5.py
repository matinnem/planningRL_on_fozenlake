...
# ---------------------------------------------------------------- algorithm
# Value iteration: apply the Bellman optimality operator
#     V_{k+1}(s) = max_a [ R(s,a,s') + γ·V_k(s') ]
# until the value function stops changing.
pi_s = {key: 5 for key in state_space}      # initial policy (all "stay")
v_s  = {key: 0.0 for key in state_space}    # initial value estimate
v_s_temp = v_s.copy()                       # for convergence check
s1ToS2 = {key: 0.0 for key in state_space}  # remembers the greedy next state
...
print("═" * 60)
print("  Value Iteration  •  FrozenLake 5x5  (Planning)")
print("═" * 60)
...
while diff >= 1e-10:
    for s_first, num_s_first in state_space.items():
        q_vals = []
        # Compute Q(s, a) for every action a.
        for sign, action in action_space.items():
            num_s_next, _ = transition(num_s_first, action)
            s_next = key_of(num_s_next)
            q_val = float(q(num_s_first, action, v_s[s_next], gamma))
            q_vals.append(q_val)
        a_max = np.argmax(q_vals) + 1                    # greedy action code
        num_s_next_final = transition(num_s_first, a_max)
        s_next_final = key_of(num_s_next_final)
        s1ToS2[s_first] = s_next_final
        pi_s[s_first]   = int(a_max)
        v_s[s_first]    = float(np.max(q_vals))          # Bellman optimality
    # L1 change across all states
    diff = np.sum([abs(v_s[key] - v_s_temp[key]) for key in v_s])
    v_s_temp = v_s.copy()
    k += 1
    render_box(v_s, pi_s, title=f"After value iteration:(k = {k})")

print()
print("═" * 60)
print(f"  Converged after {k} value-iteration steps")
print("═" * 60)
